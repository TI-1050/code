...
int main() {
	while (scanf("%d%d%d%d", &d, &na, &nb, &nc) != EOF) {
		...
		for (int i = 1; i <= na; ++i) {
			while (br < nb && b[br + 1] - a[i] <= d) ++br, up(c, nc, cl, cr, 1, b[br]);
			while (bl <= br && a[i] - b[bl] > d) up(c, nc, cl, cr, -1, b[bl]), ++bl;
			while (cr < nc && c[cr + 1] - a[i] <= d) ++cr, up(b, nb, bl, br, 1, c[cr]);
			while (cl <= cr && a[i] - c[cl] > d) up(b, nb, bl, br, -1, c[cl]), ++cl;
	//		cout << tot << " " << bl << " " << br << " " << cl << " " << cr << endl;
			res += tot;
		}
		printf("%lld\n", res);
	}
