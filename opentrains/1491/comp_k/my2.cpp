...
char ss[N], tt[N], revs[N], revt[N]; int lens, lent;
struct ExKMP {
	int Next[N];
	int extend[N];
	//下标从1开始
...
inline ll gao1() {
	...
	ll res = 0;
	exkmp[0].work(ss, tt);  
	exkmp[1].work(revs, revt);  
	reverse(exkmp[1].extend + 1, exkmp[1].extend + lens); 
	cout << (ss + 1) << " " << (tt + 1) << endl;
	cout << (revs + 1) << " " << (revt + 1) << endl;
...
void gogogo() {
	scanf("%s", s); n = strlen(s); copy(s, ss); copy(s, revs); lens = strlen(ss + 1); reverse(revs + 1, revs + 1 + lens);
	int N = strlen(s); 
	...
	REP(i, Q) {
		int a1[N + 10], a2[N + 10];
		scanf("%s", q); m = strlen(q); copy(q, tt); copy(q, revt); lent = strlen(tt + 1); reverse(revt + 1, revt + 1 + lent); 
		int M = strlen(q);
